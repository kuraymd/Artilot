...
`variants` フォルダには、同じ構成のまま雰囲気だけを変えた5種類の完成版を入れています。

- `Creator-Portfolio-Template-Cool`
- `Creator-Portfolio-Template-Dark`
- `Creator-Portfolio-Template-Girly`
- `Creator-Portfolio-Template-Minimal`
- `Creator-Portfolio-Template-RetroPop`

各フォルダは単体で配布できます。テーマごとの差分は `theme.css` にまとめてあり、`index.html` では `style.css` の後に読み込んでいます。
...
このフォルダは RetroPop テーマです。色や雰囲気は theme.css で上書きしています。
