/* ============================
 ARTILOT — JP/EN + Black/Gold Cards
 - language toggle
 - 3-card detailed result
 - 3-color palette (rounded squares)
 - history (max20) saved in localStorage
 - PNG generation (white background, includes 3 color squares)
 ============================*/

/* ---------- Data (JP + EN) ---------- */
/* We'll create parallel arrays for JP and EN (B style labels for EN) */
...
const JP = {
  ...
  colors:  ["#B00020","#1E40AF","#F2C94C","#2F855A","#7C3AED","#111827","#FFFFFF","#F2994A","#8B5CF6","#BCAAA4",
            "#00B4D8","#3A7CA5","#F7A8B8","#2D6A4F","#6B7280","#FFD166","#8338EC","#FF6B6B","#4CC9F0","#E9C46A",
            // combination groups will be generated randomly by picking three
           ]
};

// EN lists (B - tarot flavored)
const EN = {
...
function applyLanguageUI(){
  if(LANG==="JP"){
    ...
    el.clearHistoryBtn.textContent = "履歴クリア";
...
/* ---------- Core: generate a 3-card result (with both JP and EN text) ---------- */
function generateResult(){
  // pick colors: 3 distinct from colors set
  const colorsJP = pickN(JP.colors, 3);
  const colorsEN = pickN(EN.colors, 3);

  const result = {
    jp: {
      species: rand(JP.species),
      gender:  rand(JP.gender),
      age:     rand(JP.age),
      hair:    rand(JP.hair),
      cloth:   rand(JP.cloth),
      motif:   rand(JP.motif),
    ...
    en: {
      species: rand(EN.species),
      gender:  rand(EN.gender),
      age:     rand(EN.age),
      hair:    rand(EN.hair),
      cloth:   rand(EN.cloth),
      motif:   rand(EN.motif),
...
function renderResultToUI(result){
  ...
  el.card3Lines.querySelector("#c3-2").textContent = (LANG==="JP" ? "色：" : "Palette:") + data.colors.map(c=>c.toUpperCase()).join("  ");

  // palette small squares
...
/* ---------- History storage (single array, but entries contain both jp/en) ---------- */
...
/* ---------- Render history UI (display current LANG view) ---------- */
...
/* ---------- PNG generation (white background) ---------- */
function hexToRgb(hex){
  const v = hex.replace("#",""); const bigint = parseInt(v,16);
  return [(bigint>>16)&255, (bigint>>8)&255, bigint&255];
}
function generatePNGFromEntry(entry, filename="artilot.png"){
  ...
  for(let i=0;i<3;i++){
    ...
    if(i===0){
      ...
      ctx.fillText(LANG==="JP" ? "色：" : "Palette:", leftPad, curY); // draw three rounded squares next to this
      const swX = leftPad + 100;
      const swY = curY - 18;
      const swW = 40, swH = 24, swGap = 12;
      for(let k=0;k<view.colors.length;k++){
        ctx.fillStyle = view.colors[k];
        roundRect(ctx, swX + (swW+swGap)*k, swY, swW, swH, 6, true, false);
  ...
  canvas.toBlob(function(blob){
    ...
    const a = document.createElement("a"); a.href = url; a.download = `artilot_${LANG}_${Date.now()}.png`;
...
el.saveImgBtn.addEventListener("click", ()=> {
  // save current displayed result: if none displayed, use last history or generate new
  const curText = el.card1Lines.querySelector("#c1-1").textContent;
  let entry = loadHistory()[0];
  if(entry) generatePNGFromEntry(entry); else {
    const res = generateResult(); renderResultToUI(res); pushHistory(res); generatePNGFromEntry(res);
  }
});
...
el.historyList.addEventListener("click", (e)=>{
  ...
  if(!item) return;
  if(action==="restore"){
    renderResultToUI(item); // shows in current LANG view from item
    pushHistory(item); // pushing whole item
